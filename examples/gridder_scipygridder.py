...
from geoist.gridder import scipygridder

# We'll test this on the Baja California shipborne bathymetry data
data = fetch_data.fetch_baja_bathymetry()
...
# blocks.
spacing = 5 / 60
reducer = gg.BlockReduce(reduction=np.median, spacing=spacing)
...
grd = scipygridder.ScipyGridder(method="cubic").fit(proj_coordinates, bathymetry)
print("Gridder used:", grd)

# Get the grid region in geographic coordinates
region = cd.get_region((data.longitude, data.latitude))
