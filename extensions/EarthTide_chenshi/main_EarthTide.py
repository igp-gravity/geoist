...
tidex_str = '[$tidex$]'              #日潮;月潮;合成潮,分别赋值0;1;2,字符型
unit_str = '[$unit$]'                #毫咖;微咖,分别赋值0;1,字符型
tidex = int(tidex_str)                 #转换数据类型
unit = int(unit_str)                   #转换数据类型


#获取前节点数据
basepath = GetNodeBasePath()    #获取自定义节点路径
df = CsvFile2DataFrame(basepath + 'gradata.txt')   
...
df1=pd.DataFrame({"A":lon,
                  ...
                  "C":elev,
                  "D":val})
OutputTable(df1, basepath + 'EarthTideOutPut.txt')
