...
* :func:`~geoist.inversion.wavefd.elastic_psv`: Simulates the coupled P and SV
  elastic waves using the Parsimonious Staggered Grid method of Luo and
  Schuster (1990)
* :func:`~geoist.inversion.wavefd.elastic_sh`: Simulates SH elastic waves using
  the Equivalent Staggered Grid method of Di Bartolo et al. (2012)
* :func:`~geoist.inversion.wavefd.scalar`: Simulates scalar waves using simple
  explicit finite differences scheme

**Sources**

* :class:`~geoist.inversion.wavefd.MexHatSource`: Mexican hat wavelet source
* :class:`~geoist.inversion.wavefd.SinSqrSource`: Sine squared source
* :class:`~geoist.inversion.wavefd.GaussSource`: Gauss derivative source
* :func:`~geoist.inversion.wavefd.blast_source`: A source blasting in all
  directions

**Auxiliary functions**

* :func:`~geoist.inversion.wavefd.lame_lamb`: Calculate the lambda Lame
  parameter
* :func:`~geoist.inversion.wavefd.lame_mu`: Calculate the mu Lame parameter
* :func:`~geoist.inversion.wavefd.xz2ps`: Convert x and z displacements to
  representations of P and S waves
* :func:`~geoist.inversion.wavefd.maxdt`: Calculate the maximum time step for
  elastic wave simulations
* :func:`~geoist.inversion.wavefd.scalar_maxdt`: Calculate the maximum time
...
def blast_source(x, z, area, shape, amp, frequency, delay=0,
    ...
    * sourcetype : source class
        ...
        :class:`~geoist.inversion.wavefd.MexHatSource`.
...
def scalar(vel, area, dt, iterations, sources, stations=None,
    ...
    * sources : list
        ...
        (see :class:`~geoist.inversion.wavefd.MexHatSource` for an example
        source)
...
def elastic_psv(mu, lamb, density, area, dt, iterations, sources,
    ...
    * xz2ps : True or False
        ...
        :func:`~geoist.inversion.wavefd.xz2ps`.
