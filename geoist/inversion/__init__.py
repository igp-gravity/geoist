...
solvers. The main class for this is :class:`~geoist.inversion.misfit.Misfit`.
...
* :mod:`~geoist.inversion.misfit`: Defines the data-misfit classes. Used to
...
* :mod:`~geoist.inversion.regularization`: Classes for common regularizing
  functions and base classes for building new ones.
* :mod:`~geoist.inversion.hyper_param`: Classes hyper parameter optimization
  (estimating the regularization parameter), like L-curve analysis and (in the
  future) cross-validation.
* :mod:`~geoist.inversion.optimization`: Functions for several optimization
  methods (used internally by :class:`~geoist.inversion.misfit.Misfit`).
  In most cases you won't need to touch this.
* :mod:`~geoist.inversion.base`: Base classes used internally to define
  common operations and method.
  In most cases you won't need to touch this.

You can import the ``Misfit``, regularization, and hyper parameter optimization
classes directly from the ``geoist.inversion`` namespace:

>>> from geoist.inversion import Misfit, LCurve, Damping, Smoothness

The :class:`~geoist.inversion.misfit.Misfit` class is used internally in
geoist to implement all of our inversion algorithms. Take a look at the
modules below for some examples:

* :mod:`geoist.seismic.srtomo`
* :mod:`geoist.gravmag.basin2d`
* :mod:`geoist.gravmag.eqlayer`
* :mod:`geoist.gravmag.euler`
* :mod:`geoist.gravmag.magdir`
* :mod:`geoist.seismic.profile`
...
:class:`~geoist.inversion.misfit.Misfit` class.
