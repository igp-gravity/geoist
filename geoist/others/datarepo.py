...
def create(
    ...
    """
    Create a new :class:`~Drepo.Drepo` with sensible defaults to fetch data files.

    If a version string is given, the Drepo will be versioned, meaning that the local
    ...
    path : str, PathLike, list or tuple
        ...
        the end of this path. Use :func:`Drepo.os_cache` for a sensible default.
    base_url : str
    ...
    registry : dict or None
        A record of the files that are managed by this Drepo. Keys should be the file
    ...
    Returns
    -------
    Drepo : :class:`~Drepo.Drepo`
        The :class:`~Drepo.Drepo` initialized with the given arguments.

    Examples
    --------

    Create a :class:`~Drepo.Drepo` for a release (v0.1):
    ...
    try:
        ...
        warn(message)
    pup = Drepo(path=Path(path), base_url=base_url, registry=registry, urls=urls)
    return pup
...
class Drepo:
    ...
    def _download_file(self, fname):
        ...
        Used by :meth:`~Drepo.Drepo.fetch` to do the actual downloading.
