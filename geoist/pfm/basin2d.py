...
* :func:`~geoist.pfm.basin2d.Triangular`: assumes a basin with a
  triangular cross-section (think "foreland").
* :func:`~geoist.pfm.basin2d.Trapezoidal`: assumes a basin with a
  trapezoidal cross-section (think "grabben").

More complex parametrizations are:

* :func:`~geoist.pfm.basin2d.PolygonalBasinGravity`: approximate the
  basin by a polygon.
...
from . import talwani
from ..inversion.geometry import Polygon
...
class PolygonalBasinGravity(Misfit):
    ...
    :class:`~geoist.inversion.regularization.Damping`,
    :class:`~geoist.inversion.regularization.Smoothness1D`,
    :class:`~geoist.inversion.regularization.TotalVariation1D`.

    The forward modeling is done using :mod:`~geoist.pfm.talwani`.
    Derivatives are calculated using a 2-point finite difference approximation.

    .. tip::

        Use the ``estimate_`` attribute to get a
        :class:`~geoist.mesher.Polygon` version of the estimated parameters
        (attribute ``p_``).
    ...
    * props : dict
        ...
        basin :class:`~geoist.mesher.Polygon`. Ex: to give the basin a
    ...
    * xlim : None or list = [xmin, xmax]
    ...
    >>> from geoist.mesher import Polygon
    ...
    <class 'geoist.mesher.geometry.Polygon'>
    ...
    def fmt_estimate(self, p):
        """
        Convert the parameter vector to a  :class:`geoist.mesher.Polygon` so
