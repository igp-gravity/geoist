...
from geoist.snoopy.algorithms.correlator_algorithms.all import correlator_algorithms
from geoist.snoopy.anomaly_detector import AnomalyDetector
from geoist.snoopy.constants import CORRELATOR_ALGORITHM
...
class Correlator(object):
    ...
    def _get_algorithm_and_params(self, algorithm_name, algorithm_params):
        ...
        try:
            ...
            raise exceptions.AlgorithmNotFound('geoist.snoopy.Correlator: ' + str(algorithm_name) + ' not found.')
        # Merge parameters.
        if algorithm_params:
            if not isinstance(algorithm_params, dict):
                raise exceptions.InvalidDataFormat('geoist.snoopy.Correlator: algorithm_params passed is not a dictionary.')
            else:
    ...
    def _sanity_check(self):
        ...
        if len(self.time_series_a) < 2 or len(self.time_series_b) < 2:
            raise exceptions.NotEnoughDataPoints('geoist.snoopy.Correlator: Too few data points!')
