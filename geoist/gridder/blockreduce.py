...
class BlockReduce(BaseEstimator):
    ...
    Implements the :meth:`~geoist.gidder.BlockReduce.filter` method so it can be used
    with :class:`geoist.gidder.Chain`. Only acts during data fitting and is ignored
    during prediction.
    ...
    geoist.gidder.Chain : Apply filter operations successively on data.
    ...
    def _block_coordinates(self, coordinates, block_coordinates, labels):
        ...
        *block_coordinates* and *labels* should be the outputs of
        :func:`geoist.gidder.block_split`.
...
class BlockMean(BlockReduce):
    ...
    mean and not output any weights, use :class:`geoist.gidder.BlockReduce` with
    ...
    Implements the :meth:`~geoist.gidder.BlockMean.filter` method so it can be used
    with :class:`geoist.gidder.Chain`. Only acts during data fitting and is ignored
    during prediction.
